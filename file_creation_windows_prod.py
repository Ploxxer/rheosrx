...
egnyte_base_url = 'https://a4633.exemplareln.com/velox_webservice/api/'

get_url = egnyte_base_url + 'datarecord?datatype=ELNExperiment'
auth = ('' , '')
headers = {'guid':''}
...
client_url = egnyte_base_url + 'datarecord?datatype=ClientConfigurations'
...
for line in content:
    ...
    if datediff.total_seconds() < 5 * 24 * 3600:
        ...
        get_url2 = egnyte_base_url + 'datarecord?datatype=ELNExperimentDetail&field=RelatedNotebookExperiment&values=' + str(num1)
        ...
        for line2 in content2:
            ...
            if instrument_used == name:
                ...
                get_url3 = egnyte_base_url + 'datarecord?datatype=Instrument&field=InstrumentName&values=' + str(name)
...
#shut down logger to prevent multiple processes from trying to get access to test.log
logging.shutdown()
...
#can change drive letter to drive letter within sapio
#ie. from C:\\ to str(drive_letter)

if os.path.exist('C:\\log_folder'):
    print('log folder exists')
else:
    os.makedir('C:\\log_folder')
    print('log folder created')

# size is in bytes, change to control size of log files
if size > 100000:
    if os.path.exists('./test.log'):
        path = os.path.abspath('test.log')
        shutil.copy(path , drive_letter + '\log_folder\\test-'+ str(datenow) + '.log')
        os.remove('./test.log')

    dir_num = os.listdir(drive_letter + '\log_folder')

    if len(dir_num) > 3:
        path = drive_letter + '\log_folder'
        os.chdir(path)
